  ...
  passport.use('local-signup', new LocalStrategy({
    ...
    function (req, email, password, done) {
      if (email)
        email = email.toLowerCase(); // Use lower-case e-mails to avoid case-sensitive e-mail matching
      // asynchronous
      process.nextTick(function () { //makes a the process work like a queue.
        ...
        if (!req.user) { //
          User.findOne({
            ...
            // if everything is good register the user information and wait for email verification
              ...
              var emailHash = crypto.randomBytes(20).toString("hex"); //creates the unique token for the user to the email confirmation
              // create the user
